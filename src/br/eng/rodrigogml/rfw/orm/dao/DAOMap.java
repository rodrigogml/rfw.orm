...
class DAOMap {
  ...
  static class DAOMapTable {
    ...
    /**
     * Nulo na tabela t0, que representa o começo do SQL e a tabela do objeto raiz. Para as demais tabelas anexadas, contém a coluna desta tabela que será usada para fazer o join com a tabela descrita em {@link #joinAlias}.
     */
    String column = null;

    /**
     * Nulo na tabela t0, que representa o começo do SQL e a tabela do objeto raiz. Para as demais tabelas anexadas, contém a coluna da tabela descrita em {@link #joinAlias} que será usada para fazer o join com esta tabela.
     */
  ...
  static class DAOMapField {
    ...
    /**
     * Caminho do atributo a partir do objeto raiz. Vazio para os campos do objeto raiz. Não inclui o nome do atributo do objeto, este deve ser declarado em {@link #field}.
     */
    String path = null;
   ...
   * @param joinAlias Alias da tabela na qual esta fará o join. Null para a tabela raiz.
   * @param joinColumn Coluna na outra tabela (informada em 'joinAlias') utilizada para fazer o join.
   ...
   * @param joinAlias Alias da tabela na qual esta far? o join. Null para a tabela raiz.
   * @param joinColumn Coluna na outra tabela (informada em 'joinAlias') utilizada para fazer o join.
   ...
   * Essa replicação é útil quando precisamos modificar dinamicamente o DAOMap para interagir com objetos hierárquicos.
   *
   * @param originalPath Caminho original (já existente) para ser replicado. Normalmente o caminho do primeiro objeto da estrutura hierárquica.
  ...
  private DAOMapTable createMapTableForCompositionTree(String originalPath, String destinationPath, String column, String joinColumn, DAOMapTable parentTable, String baseAlias) throws RFWException {
    ...
    if (origTable != null) {
      ...
      if (newTable == null) {
        // Se ainda não existir a tabela criamos. Se existir utilizamos a existente com seu alias, mas vamos completar. Isso porque o primeiro nível de objetos hierárquicos já é mapeado, só seus subobjetos que não.
      ...
      // Buscamos todos os atributos associados a tabela base, para replicar para este novo mapeamento
  ...
  /**
   * Cria um mapeamento entre um 'field' de um objeto do sistema e uma coluna da tabela do banco de dados.
   *
   ...
   *
   * @param path Caminho desde o objeto raiz até este objeto. Não inclui o nome do atributo.
   ...
   *
   * @param fullPath Caminho desde o objeto raiz até este objeto. INCLUI o nome do atributo.
   * @return Objeto de Mapeamento entre field e coluna do banco de dados
   */
   ...
   * Mesmo que o método {@link #createMapTable(Class, String, String, String, String, String, String)} com o atributo selectFields nulo.
   *
   * @param conn Conexão com o banco para criação do PreparedStatement.
   * @param map Mapeamento do VO com as tabelas do banco.
   * @param mo Objeto com as configurações da Cláusula WHERE a ser utilizada.
   * @param orderBy Objeto com as informações de ordenação. Passar Null para não definir nenhum OrderBy.
   * @param useFullJoin Caso TRUE, o SELECT será montado com FULL JOIN ao invés do LEFT JOIN
   * @param dialect
   ...
   * @param expandTable Este atributo só é utilizado quando o selectFields não é nulo. Caso TRUE obtem todos os campos de uma tabela mesmo que só uma coluna esteja definida no selectFields. Caso FALSE recupera estritamente os campos passados em selectFields.
   * @param mo Objeto com as configurações da Cláusula WHERE a ser utilizada.
   ...
   * @param limit Define quantos registros devemos retornar da lista. Define a quantidade e não o index como o "offSet". Se ambos forem combinados, ex: offset = 5 e limit = 10, retorna os registros desde o index 5 até o index 15, ou seja da 6ª linha até a 15ª.
   * @param useFullJoin Caso TRUE, o SELECT será montado com FULL JOIN ao invés do LEFT JOIN
   * @param dialect
   * @return PreparedStatemetn pronto para realizar a consulta e obter o ResultSet.
   * @throws RFWException Lançado em caso de Erro.
  ...
  private static PreparedStatement createSelectStatement(Connection conn, DAOMap map, RFWField[] fields, String[] selectFields, boolean expandTable, RFWMO mo, RFWOrderBy orderBy, RFWField[] groupBy, Integer offSet, Integer limit, Boolean useFullJoin, SQLDialect dialect) throws RFWException {
    ...
    try {
      ...
      if (fields != null) {
        ...
        // ==> SELECT
        {
          ...
          if (selectFields == null) {
            ...
            // CACHE para armazenar as tabelas que já adicionamos para evitar de repeti-las no select. Chave é o Alias da tabela.
      ...
      if (orderBy != null) {
        ...
        // Note que essa solução só é colocada em caso de utilização do OrderBy, se não houver Order By, não precisamos do desempate já que a ordem não é importante.
      ...
      if (offSet != null || limit != null) {
        ...
        if (limit != null) {
          ...
          // Se entrou nesse IF é porque temos um offSet, mas para usar o offset o MySQL exige o limit também. Por isso utilizamos um numero gigantesco para garantir que tudo seja retornado. https://dev.mysql.com/doc/refman/8.0/en/select.html
      ...
      if (map.getMapTable().size() > 15) {
        ...
        RFW.pDev("Limite de Fetch do MySQL (Linha a Linha) habilitado!");
  ...
  public static PreparedStatement createSelectCompositionTreeStatement(Connection conn, DAOMap map, DAOMapTable startTable, Long id, RFWOrderBy orderBy, RFWField[] groupBy, Integer offSet, Integer limit, Boolean useFullJoin, SQLDialect dialect) throws RFWException {
    ...
    try {
      ...
      // ==> FROM
      {
        ...
        // Adiciona todas as tabelas que fazem join com a nossa tabela recursivamente. (para evitar um método recursivo fiz esse loop que fica reiterando até que as alterações parem
      ...
      if (map.getMapTable().size() > 15) {
        // Se tivermos mais de 15 tabelas conectadas, ativamos o fetch de linha a linha para não termos problema de memória. Não deixamos direto pq o linha a linha é pior em questões de performance.
        RFWLogger.logDebug("Limite de Fetch do MySQL (Linha a Linha) habilitado!");
        stmt.setFetchSize(Integer.MIN_VALUE);
      }
      writeStatementParameters(stmt, statementParameters);
      return stmt;
   ...
   * @param dialect
   * @return Retorna uma string no formato ").append(dialect.getQM()).append("aliasTabela").append(dialect.getQM()).append(".").append(dialect.getQM()).append("coluna").append(dialect.getQM()).append(" que associada ao atributo do VO passado como parâmetro.
   */
  ...
  /**
   * Cria o Statement SQL para inserir o mapeamento entre dois objetos em uma tabela de join.
   *
   * @param conn Conexão com o banco para criação do PreparedStatement.
   * @param map Mapeamento do VO com as tabelas do banco.
  ...
  public static <VO extends RFWVO> PreparedStatement createManyToManyInsertStatement(Connection conn, DAOMap map, String path, VO vo1, VO vo2, SQLDialect dialect) throws RFWException {
    ...
    final LinkedList<Object> statementParameters = new LinkedList<>();
    try {
      final DAOMapTable jTable = map.getMapTableByPath("." + path); // Obtem o mapeamento da tabela de join, que é colocada na Hash com mesmo caminho do path com o prefixo de "."
      final DAOMapTable mTable = map.getMapTableByPath(path);
  ...
  /**
   * Cria o Statement SQL para consultar um mapeamento entre dois objetos em uma tabela de join.
   *
   * @param conn Conexão com o banco para criação do PreparedStatement.
   * @param map Mapeamento do VO com as tabelas do banco.
   * @param path Caminho até o VO do objeto que está sendo associado no relacionamento N:N no banco.
  ...
  /**
   * Cria o Statement SQL para excluir um mapeamento entre dois objetos em uma tabela de join.
   *
   * @param conn Conexão com o banco para criação do PreparedStatement.
   * @param map Mapeamento do VO com as tabelas do banco.
   * @param path Caminho até o VO do objeto que está sendo associado no relacionamento N:N no banco.
   ...
   * @param path Caminho da tabela onde estão os elementos. No Mapeamento o caminho é criado com o caminho do atributo precedido de um ".".
   * @param id
   * @param dialect
   ...
   * @param statementParameters Lista onde serão armazenados os objetos para a posterior troca dos '?' pelos valores
   * @param dialect
   * @throws RFWException
   */
  ...
  private static StringBuilder writeWhere(DAOMap map, RFWMO mo, LinkedList<Object> statementParameters, SQLDialect dialect) throws RFWException {
    ...
    for (RFWMOData data : mo.getGreaterThanOrEqualTo()) {
      ...
      buff.append(" ").append(dialect.getQM()).append(fMap.table.alias).append(dialect.getQM()).append(".").append(dialect.getQM()).append(fMap.column).append(dialect.getQM()).append(" >= ?");
      if (encAnn != null) {
        throw new RFWCriticalException("Não é possível utilizar o operador '>=' em atributos que utilizam o valor criptografado no banco de dados. (${0}.${1}.${2})", new String[] { fMap.table.schema, fMap.table.table, fMap.column });
      } else {
        statementParameters.add(data.getValue());
      }
    }
  ...
  /**
   * Cria um statement para atualizar a coluna de FK do próprio objeto (sem alterar mais nada). Utilizado quando deixamos o objeto para atualizar a FK posteriormente (casos do INNER_ASSOCIATION).
   *
   * @param conn Conexão
   * @param map Mapeamento Objeto x Tabelas.
   ...
   * <li>Todas as tabelas de joins feitos a essa tabela, ou tabelas subsequentes serão mantidos, todos os demais removidos.
   * <li>Os Alias das tabelas continuarão sendo os mesmos (não voltarão a contagem sequencial). Assim, deixaremos de ter qualquer mapeamento com alias t0, uma vez que alias t0 é a raiz e qualquer subMap não o incluirá.
   * <li>A tabela passada como startTable automaticamente passará a ser a tabela raiz, e por tanto não terá mais a informação de "joinTable".
  ...
  public DAOMap createSubMap(DAOMapTable startTable) throws RFWException {
    ...
    // Agora que criamos todas as novas tabelas, vamos copiar os mapeamentos das colunas
